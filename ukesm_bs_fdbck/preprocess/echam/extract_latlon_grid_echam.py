...
from ukesm_bs_fdbck.constants import get_input_datapath, path_extract_latlon_outdata
from ukesm_bs_fdbck.util.imports.import_fields_xr_echam import filelist_ECHAM
...
def launch_ncks(comms, max_launches=5):
    ...
    # file = package_base_path / 'ukesm_bs_fdbck'/'preprocess'/'noresm'/'subproc_station_output.py'
    # while loop:
...
def extract_subset(case_name='SALSA_BSOA_feedback', from_time='2012-01-01', to_time='2019-01-01',
                   station='SMR',
    ...
    #lat_lims =None
    #lon_lims=None
    #case_name='SALSA_BSOA_feedback'#OsloAero_intBVOC_f19_f19_mg17_full'
    #tmp_folder = None
    #history_field='.h1.'
    ...
    fl = filelist_ECHAM(varl, case_name, path_input_data, from_time, to_time)
    input_folder = path_input_data / case_name
    # %%
    ...
    files_date = [f.split('_')[3] for f in files_stm]
    files_date = [pd.to_datetime(f, format='%Y%m') for f in files_date]
    ...
    # case_name='case_name'
    for v in varl:
        files_str_patt = f'{f.parent}/{case_name}*{v}_tmp_subset.nc'
