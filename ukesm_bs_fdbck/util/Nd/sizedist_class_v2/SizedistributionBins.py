...
from ukesm_bs_fdbck.util.Nd.sizedist_class_v2.SizedistributionSurface import SizedistributionSurface
from ukesm_bs_fdbck.util.collocate.collocate import CollocateModel
from ukesm_bs_fdbck.util.filenames import get_filename_pressure_coordinate_field, get_filename_ng_field

from ukesm_bs_fdbck.data_info.variable_info import sized_varListNorESM
from ukesm_bs_fdbck.util.imports.get_pressure_coord_fields import get_pressure_coord_fields
from ukesm_bs_fdbck.util.Nd.sizedist_class_v2 import Sizedistribution
from ukesm_bs_fdbck import constants
# import ukesm_bs_fdbck.util.Nd.sizedist_class_v2.constants as constants_sizedist
import ukesm_bs_fdbck.util.Nd.sizedist_class_v2.constants as constants_sizedist
from ukesm_bs_fdbck.util.practical_functions import make_folders
# from ukesm_bs_fdbck.util.naming_conventions.var_info import get_varname_Nd
import numpy as np
...
class SizedistributionStationBins(SizedistributionSurfaceBins):
    ...
    def __init__(self, *vars,
                 diameters=constants_sizedist.diameter_obs_df, **kwargs):
        self.diameters = diameters
    ...
    def get_Nd_output_name(self, out_varn):
        fn = self.coll_mod.savepath_coll_ds(
            out_varn)  # (out_varn, self.model_name, self.case_name, self.from_time, self.to_time)
        make_folders(fn)
        return fn
