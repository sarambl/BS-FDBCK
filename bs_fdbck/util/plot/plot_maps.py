...
# from bs_fdbck.util.plot import  plot_settings
from bs_fdbck.data_info import get_nice_name_case, get_area_specs
from bs_fdbck.util.slice_average.area_mod import lon180_2lon360
from bs_fdbck.util.slice_average.avg_pkg import average_model_var  # , get_lat_wgts_matrix, masked_average
...
def plot_map(var, case, cases_dic, figsize=None,
    ...
    label = get_fancy_var_name(var) + ' [%s]' % bs_fdbck.util.naming_conventions.var_info.get_fancy_unit_xr(plt_var,
                                                                                                                var)
    ...
    unit = bs_fdbck.util.naming_conventions.var_info.get_fancy_unit_xr(plt_var, var)
...
def plot_map_diff(var,
    ...
    if relative:
        ...
        func = fdifference
        unit_diff = ' [%s]' % bs_fdbck.util.naming_conventions.var_info.get_fancy_unit_xr(ctrl_da, var)
...
def get_avg_diff(case_oth, case_ctrl, case_dic, relative, var):
    ...
    if relative:
        ...
        unit = bs_fdbck.util.naming_conventions.var_info.get_fancy_unit_xr(case_dic[case_ctrl][var], var)
