...
def make_fn_scat(case, v_x, v_y):
    _x = v_x.split('(')[0]
    _y = v_y.split('(')[0]
    f = f'scat_all_years_f09_{case}_{_x}_{_y}.png'
    return plot_path /f
...
# %%
cases_orig1 = ['OsloAero_intBVOC_f09_f09_mg17_full']
cases_orig2 = ['OsloAero_intBVOC_f09_f09_mg17_ssp245']
# %%
...
varl =['N100','DOD500','DOD440','ACTREL',#,'SOA_A1',
       ...
       'NMR01','SIGMA01',
       'NCONC02','NCONC03','NCONC04','NCONC05','NCONC06','NCONC07','NCONC08','NCONC09',
       'NCONC10','NCONC11','NCONC12','NCONC13','NCONC14',
       #'NCONC01',# 'N50','N100',
...
# %%
#_ds1['N50'].sel(station='SMR').isel(lev=-1).plot()
#_ds1['NCONC01'].sel(station='SMR').isel(lev=-1).plot()
tot = 0
for i in range(1,15):
    s = '%02.0f'%i
    _ds1[f'NCONC{s}'].sel(station='SMR').isel(lev=-1).plot()
    tot = tot + _ds1[f'NCONC{s}'].sel(station='SMR').isel(lev=-1)
    
tot.plot(c='k',alpha=.3)
#_ds1['N50'].sel(station='SMR').isel(lev=-1).plot()

# %%
(2*_ds1['NMR01']).sel(station='SMR').isel(lev=-1).plot()


# %%

# %%
(2*_ds1['SIGMA01']).sel(station='SMR').isel(lev=-1).plot()
