...
from bs_fdbck.preprocess.launch_monthly_h1 import launch_monthly_h1
from useful_scit.imps import *

from bs_fdbck.data_info.variable_info import list_sized_vars_noresm
from bs_fdbck.util.imports.get_fld_fixed import get_field_fixed, merge_monthly_flds
...
for case_name in cases_sec:
    #continue
    launch_monthly_h1(case_name, True, from_time=from_t, to_time=to_t)
for case_name in cases_orig:
    #continue
    launch_monthly_h1(case_name, False, from_time=from_t, to_time=to_t)

# %%
for case_name in cases_sec:

    merge_monthly_flds(case_name, varl, from_t, to_t,
                       pressure_adjust=False, model = 'NorESM', history_fld=history_field, comp='atm', chunks=None,
                       get_constants=True)
...
for case in cases:
    get_field_fixed(case,varl, from_t, to_t, #raw_data_path=constants.get_input_datapath(),
                    pressure_adjust=False, model = 'NorESM', history_fld=history_field, comp='atm', chunks=None)
