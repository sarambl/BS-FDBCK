...
from bs_fdbck_clean.data_info.variable_info import import_constants as import_constants_list
from bs_fdbck_clean.util.filenames import get_filename_constants
...
def xr_import_NorESM(case, varlist, from_time, to_time, path=constants.get_input_datapath(),
    ...
    if varlist is not None:
        ...
        drop_list = list((set(dummy.data_vars.keys()) - set(var_names_mod)) - set(
            bs_fdbck_clean.data_info.variable_info.import_always_include))
        dummy.close()
    else:
        drop_list = None
    # Read the files:
