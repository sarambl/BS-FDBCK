...
def make_fn_scat(case, v_x, v_y):
    _x = v_x.split('(')[0]
    _y = v_y.split('(')[0]
    f = f'scat_all_years_2stations_{case}_{_x}_{_y}_atto_lev{i_lev_atto}.png'
    return plot_path /f
...
for mo in di_mod2cases.keys():
    for ca in di_mod2cases[mo]:
        ...
        fp_atto = path_save_daily_medians_ATTO.parent / f'{path_save_daily_medians_ATTO.name}_{use_name}-ilev{i_lev_atto}.csv.csv'
...
# %%
pd.DataFrame(dic_fits['ATTO']['OA'])#.loc['popt'].Z
...
df_all.to_csv(f'Plots/Both_stations/all_fits-lev_atto{i_lev_atto}.csv')
